...
static switch_bool_t amd_read_audio_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
    ...
    switch (type) {
        ...
        case SWITCH_ABC_TYPE_CLOSE:
        {
            ...
            detected = switch_channel_get_variable(vad->channel, "amd_result");
            //cause = switch_channel_get_variable(vad->channel, "amd_cause");
            uuid = switch_core_session_get_uuid(vad->session);

            if (!strcasecmp(detected, "HUMAN")) {
                switch_log_printf(
                    SWITCH_CHANNEL_SESSION_LOG(vad->session),
                    SWITCH_LOG_DEBUG,
                    "AMD Detected Result: HUMAN\n");
            }
            if (!strcasecmp(detected, "MACHINE")) {
                switch_log_printf(
                    SWITCH_CHANNEL_SESSION_LOG(vad->session),
                    SWITCH_LOG_DEBUG,
                    "AMD Detected Result: MACHINE\n");

                /* AK */
                // http://asterisk.voicemeup.com/viewtopic.php?p=61706&sid=6b1ca35d89ae4663b593fb872b1dc176
                switch_core_session_execute_application(vad->session, "set", "api_result=${uuid_setvar ${aleg_uuid} sip_bye_h_X-Remote-Reason AMD}");

                switch_ivr_kill_uuid(uuid, 45);
            }
